...
aiOrb.addEventListener('click', (e) => {
  ...
  if (chatOpen) {
    ...
    openChat();
    // First time the orb is clicked: show the intro message instantly,
    // right alongside the panel opening (no separate global trigger).
...
// FIX: uses double requestAnimationFrame instead of setTimeout(100)
// so the fade-in triggers on the very next paint (feels instant)
// while still animating smoothly instead of popping in abruptly.
function showIntroMessage() {
  introMsg.style.display = 'block';
  introMsg.style.opacity = '0';
  introMsg.style.transform = 'translateY(6px)';
  introMsg.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
  ...
  // First time only: show all suggestion options directly, expanded.
  // After the user picks one (or asks their own question), it
  // collapses down to just the "More questions" toggle.
  if (chatSuggestions && suggestionsToggle) {
    chatSuggestions.classList.add('open');
...
function collapseSuggestions() {
  if (chatSuggestions) chatSuggestions.classList.remove('open');
  if (suggestionsToggle) suggestionsToggle.classList.remove('open');
}
function toggleSuggestions() {
  if (!chatSuggestions) return;
  const isOpen = chatSuggestions.classList.toggle('open');
  if (suggestionsToggle) suggestionsToggle.classList.toggle('open', isOpen);
}
if (suggestionsToggle) suggestionsToggle.addEventListener('click', toggleSuggestions);

// Suggestion buttons — clicking one asks the question, then
// collapses the list back down to just the "More questions" button.
...
/* ======================================================
   12. INSTAGRAM REEL EMBED FIX
   (Forces Instagram's embed.js to fully re-process the
   reel blockquotes so they don't get stuck half-loaded)
   ====================================================== */
...
window.addEventListener('load', () => {
  // First attempt shortly after full page load
  setTimeout(processInstagramEmbeds, 500);
  // Second attempt as a safety net in case embed.js
  // was still finishing its own script injection
  setTimeout(processInstagramEmbeds, 1800);
});